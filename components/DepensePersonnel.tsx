  ...
  const renderSettle = () => {
    ...
    let usersCopy = userList;
     ...
     while(usersCopy.length >= 2) {//tant qu'il existe des gens av des dettes pos ou neg 
      let maxEnHess = usersCopy.pop() //
      let maxEnBenef = usersCopy[0]
      if(!(-0.1 < maxEnBenef.Balance && maxEnBenef.Balance < 0.1)){
      debtList.push(<Dette amount={-maxEnHess.solde} deveur={maxEnHess.nom} receveur = {maxEnBenef.nom} key ={maxEnHess.uuid}/>)//on ajoute que le plus en hess rembourse toute sa dette a le plus en benef
